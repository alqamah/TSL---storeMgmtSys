...
> [!IMPORTANT]
> **Schema Design Decisions** — Please confirm the following before I proceed:
>
> 1. **Issue tracking**: The `issue` schema has a single `item` and `employee` per record. Should one issue be able to cover **multiple items** (like a checkout cart), or is one-item-per-issue correct?
> 2. **Return workflow**: When an item is returned, should we update the `return_date` on the existing issue record, or create a separate "return" record?
> 3. **Authentication**: Do you need user login/authentication (e.g., only authorised issuers can issue items), or is this an open internal tool for now?
> 4. **Item availability**: Should the system track current quantity/availability (e.g., `capacity` = total units, and we decrement on issue), or is each `Item` a unique physical asset (1:1)?

---

## Proposed Changes

### Phase 1: Project Scaffolding & MongoDB Schemas ← *current phase*
...
### Automated Tests
- Schema validation tests (invalid `p_no`, `phone`, duplicate `sap_id`)
- API endpoint tests with Supertest
- Frontend component tests with React Testing Library

### Manual Verification
- Postman/curl for all CRUD endpoints
